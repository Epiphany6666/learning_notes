...
`FC(Function Component)` 是React中提供的类型，并且FC是一个泛型，后面可以通过 `<>` 传入一个类型变量，传入的这个类型变量实际上就是去指定组件能够接收的属性的。

在这个例子中就是说：组件能接收 `name属性` 和 `age属性`，并且age是可选的，这样的话将来在我们渲染 `Hello组件` 的时候，`name属性` 必须有值的，为字符串类型，但由于age是可选的，因此age可以省略。

同时我们指定的 `Props` 也作为了函数参数的类型，因此我们在使用函数参数的时候，就能直接使用对象的解构。然后就可以在jsx中使用name和age了。

<img src="./assets/image-20240521122620845.png" alt="image-20240521122620845" style="zoom:67%;" />

在进行书写的时候，可以先将函数的基本结构写好，然后再回到函数名称的位置通过 `:(冒号)` 来指定类型

![image-20240521123107400](./assets/image-20240521123107400.png)
...
组件类型指定好了后，会有一个报错，错误就是告诉我们：`不能将类型“() => void”分配给类型“FC<Props>”。
  不能将类型“void”分配给类型“ReactNode”。`。

解释：FC约定了函数是一个React组件，既然是React组件，就需要返回内容，返回的类型应该是 `ReactNode类型`，现在什么都不返回，肯定会报错。

<img src="./assets/image-20240521124252565.png" alt="image-20240521124252565" style="zoom:77%;" />

如果想要拿到组件的属性，直接通过参数来拿即可。
