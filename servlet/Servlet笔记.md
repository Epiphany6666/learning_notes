...
5. 找到`servlet`中的`servlet-class`就能找到`com.atguigu.servlets.AddServlet`类
...
当你把Web模块加入进项目后，说明你是个Web项目，既然是个web项目，将来就需要部署到tomcat上，但是部署到tomcat上时需要一个部署的包，但你现在还没有这个部署的包，它就问你要不要生成这个部署的包，点击生成即可。

![image-20240303201227502](assets/image-20240303201227502.png)

就算这里没有点击，也可以通过另一种方式创建Artifact

> 下面的归档（Archive）就是压缩包，jar包是java应用程序的压缩包。
>
> Web Application:Archive：war包就是Web应用的压缩包，将该压缩包仍到tomcat中，就有一个叫webapps。
>
> 当你把war包仍进去后，tomcat会自动给你解压。
>
> 我们也可以不扔一个压缩包，我们仍一个压缩包解压之后的，就是Web Application:Exploded
...
我们已经知道了servlet是线程不安全的，给我们的启发是：尽量的不要在servlet中定义成员变量。如果不得不定义成员变量，那么 ①不要去修改成员变量的值  ②不要根据成员变量的值做一些逻辑判断。
...
2. 新建文件夹lib_thymeleaf
   ...
   ![image-20240305170923847](assets/image-20240305170923847.png)
