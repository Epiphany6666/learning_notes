...
# 59.利用ref 和 $refs获取 dom 元素 或 组件实例
...
<img src="./assets/1682414342139.png" alt="68241434213" style="zoom:67%;" />
...
<img src="./assets/1682442945057.png" alt="68244294505" style="zoom:67%;" />
...
解决办法：目的就是告诉Vue组件的名字是由多个单词组成的，以 `Friend组件` 为例
...
>
> <img src=".\assets\image-20240205105050742.png" alt="image-20240205105050742" style="zoom: 67%;" />

```js
// 当保存的时候，eslint自动帮我们修复错误，codeActionsOnSave：代码在保存的时候，fixAll：修复所有错误
...
+ 注意：使用了eslint校验之后，把vscode带的那些格式化工具全禁用了 Beatify，即不再使用其内置或已安装的其他格式化插件

----

## 二、注意

ESLint插件并不是说可以将所有的错误帮我们解决，有一些是跟代码编写相关的，这个就需要我们自己手动解决。因此手动解决和自动解决这两个是都需要掌握的，例如声明变量不使用就会报错。

<img src="./assets/image-20240530132834773.png" alt="image-20240530132834773" style="zoom:67%;" />

此时就需要手动使用

![image-20240530132810470](./assets/image-20240530132810470.png)
...
<img src="./assets/1683178293366.png" alt="68317829336" style="zoom:67%;" />
...
<img src=".\assets\image-20240205120645447.png" alt="image-20240205120645447" style="zoom:67%;" />
...
> Vue2所使用的插件是Vetur，Vue3所使用的插件是volar，现在改名为 `Vue-official` 了
>
> 首先将Vetur给禁用，然后安装volar，然后重启VSCode即可
>
> <img src=".\assets\image-20240213194022040.png" alt="image-20240213194022040" style="zoom:67%;" />
...
<img src="./assets/5.png" alt="image.png" style="zoom: 50%;" />
...
> Vue2中 `v-model` 相当于 `:value和@input`
>
> Vue3是进行了统一，让我们的记忆负担更小一些

## 一、引入

在Vue3中，自定义组件上使用v-model, 相当于传递一个`modelValue`属性，同时触发 `update:modelValue` 事件
...
> 使用工具选中标签
>
> ![image-20240219171811903](.\assets\image-20240219171811903.png)
...
<img src="./assets/33.png" alt="image.png" style="zoom:57%;" />
...
<img src="./assets/34.png" alt="image.png" style="zoom:67%;" />

需求：在Pinia中获取频道列表数据并把数据渲染App组件的模板中
<img src="./assets/35.png" alt="image.png" style="zoom:67%;" />
...
<img src="./assets/36.png" alt="image.png" style="zoom:37%;" />
...
![image-20240214201147987](.\assets\image-20240214201147987.png)
...
![image-20240219113603685](.\assets\image-20240219113603685.png)

规则请参考接口文档

<img src=".\assets\image-20240219113803828.png" alt="image-20240219113803828" style="zoom:50%;" />
...
![image-20240219112601065](.\assets\image-20240219112601065.png)
...
![image-20240219133801531](.\assets\image-20240219133801531.png)
...
![image-20240219134016231](.\assets\image-20240219134016231.png)
...
![image-20240219142327706](.\assets\image-20240219142327706.png)
...
![image-20240219145813679](.\assets\image-20240219145813679.png)

可以看见`el-config-provider`报过来el-table等，即它允许你对局部的组件进行中英文切换

![image-20240219150117151](.\assets\image-20240219150117151.png)
...
>
> <img src=".\assets\image-20240219174050129.png" alt="image-20240219174050129" style="zoom:50%;" />
...
![image-20240219203637070](.\assets\image-20240219203637070.png)

将代码直接复制粘贴

![image-20240219203732986](.\assets\image-20240219203732986.png)

可供选择的列表项

![image-20240219204337014](.\assets\image-20240219204337014.png)

layout的顺序和展示出来的顺序都是一一对应的

![image-20240219204634955](.\assets\image-20240219204634955.png)

两个函数都能接收到参数，并且都是number类型

![image-20240219205048462](.\assets\image-20240219205048462.png)

整个el布局是加了flex布局的

![image-20240219205605056](.\assets\image-20240219205605056.png)
...
![image-20240219215403122](.\assets\image-20240219215403122.png)

![image-20240219215523079](.\assets\image-20240219215523079.png)

修改方向

![image-20240219222719055](.\assets\image-20240219222719055.png)

改变大小

![image-20240219223240870](.\assets\image-20240219223240870.png)
...
>
>    ![image-20240220001218779](.\assets\image-20240220001218779.png)

element-plus实际上是支持默认文件上传的。

![image-20240220001712576](.\assets\image-20240220001712576.png)

name的默认值是file，如果需要修改，可以通过method修改

![image-20240220002018379](.\assets\image-20240220001947468.png)
...
> 关闭自动上传
>
> ![image-20240220002657230](.\assets\image-20240220002657230.png)
>
> 
>
...
>
> ![image-20240220004325985](.\assets\image-20240220004020515.png)
>
> 打印第一个对象，File对象就是这个raw，如果我们要创建预览地址时，需要基于这个raw createObjectURL，并且把这个值给imgURL
>
> ![image-20240220004533977](.\assets\image-20240220004533977.png)
...
![image-20240220103603346](.\assets\image-20240220103603346.png)
...
>
> <img src=".\assets\image-20240220103850484.png" alt="image-20240220103850484" style="zoom: 33%;" />
...
>
> ![image-20240220120446991](.\assets\image-20240220120446991.png)
...
![image-20240220150413561](.\assets\image-20240220150413561.png)
