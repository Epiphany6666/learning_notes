...
```js
const fs = require('fs')
console.log(__dirname) // D:\备课代码\2_node_3天\Node_代码\Day01_Node.js入门\代码\03（即当前项目的绝对路径）
...
> `name`：用来记录当前所在文件夹的包的名字，这个包的名字是你要将当前文件夹的包发布到网上才需要使用这个名字
>
> 许可证其实就是一个英文的固定名字的字符串的值。MIT就是一个许可证，它的意思就是，如果这个软件包以后商用的话，就可以使用当前作者的名字进行一个宣传，所以它就是一个标记而已。

![image-20230331154639881](./assets/image-20230331154639881.png)

包使用：在引入一个包文件夹到代码中，只需要写到包文件夹就行了（不理解的话结合下面示例看），代码运行时默认引入的是包文件节下的 index.js 模块文件里导出的对象，如果没有 index.js 文件，则会引入 package.json 里 main 属性指定的文件模块导出的对象
...
2. 下载软件包：npm i 软件包名称
   ...
   >
   > 下载的包的源代码会存放在当前项目下的 node_modules 中，并记录在 package.json 中
...
软件包区别：
* 本地软件包：当前项目内使用，<span style="color: red;">封装属性和方法</span>，包的源代码存在于 node_modules

* 全局软件包：本机所有项目使用，<span style="color: red;">封装命令和工具</span>，存在于系统设置的位置

  > 全局软件包封装的是命令和工具，只有这样的包才能安装到全局环境中（即 -g 参数）

这节学习的全局软件包叫nodemon，作用：带来了一个新的命令 `nodemon` ，主要是替代 node 命令，它可以检测代码更改，然后自动重启程序。

使用：
1. 安装：npm i nodemon -g （-g 代表安装到全局环境中）

   由于是全局安装，因此在哪个目录下的终端执行安装命令都没关系。

2. 运行：nodemon 待执行的目标 js 文件

下载成功后它会提醒你下载了32个软件包，这是因为它可以检测到nodemon又依赖的包，此时会一并下载

需求：使用 nodemon 命令来启动任意一个，然后修改代码保存后，观察终端是否会重启应用程序

~~~js
nodemon demo1.js
~~~

执行命令后，可以发现它启动了一个服务，因为在终端中运行完并没有结束

<img src="./assets/image-20240517082530098.png" alt="image-20240517082530098" style="zoom:67%;" />
...
下图是Webpack官网对它做出的定义。Webpack 是一个静态模块打包工具，从入口构建依赖图，打包有关的模块，最后用于展示你的内容。
...
打包：把静态模块内容读到代码里面，然后进行压缩，整合，转译等（前端工程化）

> 整合：将多个js文件或者多个css文件整合成一个，可以减少用户浏览器的http请求次数，从而可以使用户更快的访问我们的网页

- 翻译转义：把 less / sass 转成 css 代码（有了Webpack静态模块打包工具后，就不用下载插件也能实现了）
- 把 ES6+ 降级成 ES5
- 支持多种模块标准语法

Webpack静态模块打包工具有了后，就可以针对前端代码进行工程化的打包处理。
...
3. **下载 webpack和webpack-cli 到项目（版本独立）**

   它实际上是一个全局的软件包，只不过这里我们先将它下载到当前项目中，这是因为本机中可能有多个项目，多个项目的Webpack版本我们需要让它分别独立。

   > --save-dev：主要用来标记，这两个包是在开发环境中使用
   >
   > 使用npm下载时，两个软件包之间使用空格隔开。

   ```bash
   npm i webpack webpack-cli --save-dev
   ```

   下载完毕后，配置到的是`devDependencies`中，如果不设置开发环境，则就直接配置到`dependencies`中，就是这样的一个区别

   <img src=".\assets\image-20240127110722542.png" alt="image-20240127110722542" style="zoom:67%;" />

4. **自定义命令**

   下完了两个软件包后，它给我们带来了 `webpack` 打包命令，但是这个命令我们想要在这个局部环境下使用，此时就需要配置到当前项目的 `package.json` 中，然后才会对当前项目生效。

   PS：前面的 `key` 叫做自定义命令，可以随便写；后面的value就是运行这个自定义命令后，真正在终端里要出发的命令，名字是固定的，叫做 `webpack`，这个就是这个软件包提供的打包命令了。

   ![image-20230403110640647](./assets/image-20230403110640647.png)

5. **项目中运行工具命令，采用自定义命令的方式（也叫作局部命令）**
   ...
   可以发现真正执行的命令还是 `webpack`。执行后会有警告（红色的并不是错误，因为它是属于警告的范围），没有errors就说明打包成功了，圈起来的是打包信息。

   ![image-20240520161734707](./assets/image-20240520161734707.png)

6. **打包后自动产生 dist 分发文件夹（压缩和优化后，用于最终运行的代码）**

   webpack会分析代码里运作的过程和结果，它发现代码运行的结果只是为了打印true或者false，所以webpack会对我们的代码进行转化和翻译，最终打印一个true或false就完成了。

   注释、换行符，函数都给过滤掉了，将代码做了一个极致压缩。
...
Webpack默认入口是 `src/index.js`，默认出口是 `dist/main.js`。

如何影响 Webpack 打包过程：查文档（文档才是我们最好的老师，文档里没有的时候可以通过查阅百度查看其他的文章结合看），新建配置文件和配置属性
...
![image-20240520164934624](./assets/image-20240520164934624.png)

那么 `Webpack.config.js文件` 应该建在哪里呢？查阅百度，Webpack.config.js 应该放在项目的根目录下。

bundle：捆、包，也就是Webpack分析打包输出后的模块文件

![image-20240520165258254](./assets/image-20240520165258254.png)

但是在 `Webpack.config.js` 配置文件中，`module.exports` 只能有一次，因此在刚刚 `entry` 的下面添加 `output` 属性配置对象即可。

- `out.path`：表示输出到哪一个文件夹的下面
- `out.filename`：打包后那个文件的名字
...
这一次我们在文档中要找的是 `Plugin插件`，在插件中，可以在左侧的目录中找到 `HtmlWebpackPlugin` 插件。

> 这个插件实际上是本地的一个软件包，所以需要先把它下载下来，然后也是在开发环境中使用，`--save-dev`命令行参数可以加在前面或者后面都可以

该插件需要用过 `require` 进行引入，然后在配置对象中使用一个新的名字，叫做 `plugins选项`，这个选项就是给 `Webpack` 添加插件的功能，我们直接 `new` 这个插件的构造函数。

![image-20240520171539630](./assets/image-20240520171539630.png)

接下来插件就会为我们在 `dist目录` 下生成 `index.html文件`，在这个文件中会自动引入我们打包输出的 `js文件`，这就是这个插件的能力。

![image-20240520171948561](./assets/image-20240520171948561.png)

但是我们前面做的是一个登录的页面，而这个插件默认生成的html中，它并没有我们登录页面需要的标签，因此这个文档看到这里其实还不够，我们还需要接着往下找。

点击 `插件文档`

<img src="./assets/image-20240520172155674.png" alt="image-20240520172155674" style="zoom:67%;" />

此时就会来到GitHub中插件对应的页面中。例如 `title`：影响的是插件生成的HTML文档中的浏览器上的title标题

我们可以在这个文档中一个一个去查找和查看，也可以结合百度其他文章结合来看，这里为了节省时间，就直接告诉大家。

我们需要找的配置项叫做 

- `template`，使用 `template` 就可以指定这个插件以哪个html文件作为模版基础，在它的基础内容上，再去生成 `dist` 下的网页。

- 还有一个选项叫 `fileName`，这个属性就可以指定生成的 `文件夹的路径 和 html网页的名字`

![image-20240520172535663](./assets/image-20240520172535663.png)

然后再往下翻，结合官方示例来书写：往构造函数中传入配置对象即可。

![image-20240520172618157](./assets/image-20240520172618157.png)
...
4. 生成的html文件中引入的JS路径是这样的：
   ...
   这是因为我们的html-webpack-plugin插件里面的源代码，它会根据filename的路径，从当前路径开始分析，.代表的是login文件夹，然后它会..，看看是否匹配到出口路径中的**dist文件夹的名字**，发现名字一致，就直接在../的基础上拼接filename输出的打包后的文件路径。
...
因此，想让有Webpack有更多的功能：就去官网找包，下包，安装文档配置包
...
> 之前学的插件只是为了让Webpack有更多的功能，因此插件和加载器的目的是不一样的。

介绍需要的 2 个加载器来辅助 Webpack 才能打包 css 代码

* [加载器 css-loader](https://webpack.docschina.org/loaders/css-loader/)：解析 css 代码
* [加载器 style-loader](https://webpack.docschina.org/loaders/style-loader/)：把解析后的 css 代码插入到 DOM页面（style 标签之间）

可以发现这里的文档还是挺多的，但是这里我们先来做一个入门的使用，因此它更多使用的场景大家以后工作的时候用到了就可以来这边查找一下。

我们先来看一下 `快速开始`。

`如果要使用 css-loader，你需要安装 webpack@5`：webpack@5也就是Webpack5的版本，如果你是webpack4的版本，那么你也要找到与之对应的版本来进行安装，安装时如果不指定版本，那么都是默认最新的。

这里它在配置文件中新增了一个 `module` 的配置对象，这个 `module` 就是我们要放里面一些加载器的配置项。

`module对象` 中还有一个属性 `rules(规则)`，因此这两个属性都是webpack规定好的，因此不要乱写。

在 `rules数组` 中就是每一个加载器使用的规则对象，`test` 和 `use` 这两个属性也是固定的。

- `test` ：匹配与 `.css` 结尾的文件
- `use`：当匹配test后，它就会使用对应的加载器去加载文件里面的内容。
- 加载器的顺序也需要注意，它会从后往前进行使用，因此我们千万不要颠倒它的顺序。

这里还有最重要的一步：让css文件跟入口文件产生引入关系，这是因为Webpack会找到入口，会读取入口文件内的代码，看看引入了哪些其他模块文件，然后才会把对应模块文件里的代码构建起来，一起打包输出到我们指定的出口，这里就是 `dist/login/index.js`。

![image-20240520174656446](./assets/image-20240520174656446.png)
...
1. 准备 css 文件引入到 `src/login/index.js` 中（压缩转译处理等）
   ...
   这个路径也可以通过 `node_modules` 中找到：`node_modules\bootstrap\dist\css\bootstrap.min.css`

   ![image-20240520181612720](./assets/image-20240520181612720.png)
   ...
   > 注意：`import './index.css'`：这里只是引入代码内容让 Webpack 处理，不需定义变量接收在 JS 代码中继续使用，并且就算使用变量接收，在下面也用不上，所以可以省略变量和 `from` 的书写，直接引入即可

   ![image-20240520175712107](./assets/image-20240520175712107.png)
...
打包后发现dist目录中生成的html并没有引入css文件的动作

![image-20240520181027354](./assets/image-20240520181027354.png)

但是别忘了我们刚刚说过，CSS代码是打包输出到index.js出口。
   ...
   module.exports = {
     ...
     module: {
       rules: [
         {
           test: /\.css$/i, // 匹配 .css 结尾的文件
           use: [MiniCssExtractPlugin.loader, 'css-loader'], // 从后往前使用加载器来解析 css 代码和插入到 DOM
         },
         {
           test: /\.less$/i,
...
PS：这个文档跟你之前看见的文档不太一样，它需要你仔细的配合其他的资料认认真真的看。

Webpack默认不能识别图片这样的代码，因为Webpack默认只能识别JS代码内容，或者JSON格式代码内容。

[资源模块](https://webpack.docschina.org/guides/asset-modules/)：但是从Webpack5 内置了资源模块的打包，无需下载额外 loader ，只需要给Webpack进行一些额外的配置即可。

资源模块(asset module)是一种模块类型，它允许使用资源文件（字体 `font`，图标 `svg`，图片，视频，音频等）而无需配置额外 loader

但如果你是Webpack5的版本，就需要使用下面的几个loader自己配置，但是由于我们现在使用的是Webpack5最新的版本，所以就继续往下看。

![image-20240127161741783](.\assets\image-20240127161741783.png)

我们可以设置Webpack5打包时的资源模块的类型，即Webpack5如何处理这些资源文件模块，一共有四种方式。

~~~js
资源模块类型(asset module type)，通过添加 4 种新的模块类型，来替换所有这些 loader：
// 意思就是把代码里面引的图片等资源模块文件复制到输出的目录下，然后再把我们代码中引入资源路径的地方换成URL网址的格式，这种方式是默认的
...
// 在导出一个 data URI 和 发送一个单独的文件之间（即第一种和第二种） 之间进行一个自动选择，判断方式这里没说，但是提到了：配置资源体积限制实现。大家可以根据百度查找到的其他资料就知道了，这里直接告诉大家。
// 它会判断我们引入的资源模块文件是否在8kb的大小，如果大于8kb，那它就会直接复制文件，如果小于8kb，它会把文件内容转成data URI格式，把我们资源模块内容打包进JS代码中
asset 在导出一个 data URI 和发送一个单独的文件之间自动选择。之前通过使用 url-loader，并且配置资源体积限制实现。
~~~

接着往下看，仔细看文档可以发现，它使用的是旧的办法，按它这样写其实也能实现，但是我们可以找找有没有新式的写法。

但是可以参考它模式的选择，它这里将asset这个模块的类型设置为了 '` javascript/auto` ' ，由此就能推导出，它由`type`字段来进行设置，那我们就直接找 `type为asset` ，让它在这种类型下进行打包。

<img src="./assets/image-20240521144903889.png" alt="image-20240521144903889" style="zoom:67%;" />
...
此时它匹配到的就是通用类型的位置，类型为 `asset` 没错，但是它匹配的是 `.txt` 结尾的文件。

![image-20240127163449217](.\assets\image-20240127163449217.png)

那我不想要匹配 `.txt` 怎么办？

此时我们就可以结合我们刚刚最上边的配置：匹配以这些类型的文件 `/\.(png|jpg|gif)$/i` 结尾的文件，

<img src=".\assets\image-20240127163629191.png" alt="image-20240127163629191" style="zoom: 50%;" />

---

如果我们想要自定义输出的路径，需要找到 `自定义输出文件名`

可以通过在 webpack 配置中设置 `output.assetModuleFilename` 来修改此模板字符串

![image-20240521151239310](./assets/image-20240521151239310.png)

往下看，还有另一种自定义输出文件名的方式。

其中 `[hash][ext][query]` 是占位符。

<img src="./assets/image-20240521152430876.png" alt="image-20240521152430876" style="zoom:60%;" />

----

## 二、查看文档总结

文档多切繁琐，我们来总结一下我们刚刚看见的内容。

[资源模块](https://webpack.docschina.org/guides/asset-modules/)：从Webpack5 内置了资源模块的打包，无需下载额外 loader，只需要给Webpack进行一些额外的配置即可。

从 `rules` 加载器规则对象中，根据我们刚刚找到的文档，`test` 的作用是：以拿些文件结尾时，对这些资源模块处理。

这里并没有使用 `use属性`，而是使用 `type属性` 来控制资源模块的打包类型，这里选择 `asset`，让它自己判定是那两种方式的哪一种。

~~~ts
// 在导出一个 data URI 和 发送一个单独的文件之间（即第一种和第二种） 之间进行一个自动选择。
// 它会判断我们引入的资源模块文件是否在8kb的大小，如果大于8kb，那它就会直接复制文件，如果小于8kb，它会把文件内容转成data URI格式，把我们资源模块内容打包进JS代码中
asset 在导出一个 data URI 和发送一个单独的文件之间自动选择。之前通过使用 url-loader，并且配置资源体积限制实现。
~~~

<img src="./assets/image-20240521153807994.png" alt="image-20240521153807994" style="zoom:67%;" />
...
> 发送一个单独的文件：所以它就把代码里引的图片或者字体等等文件，它就会直接复制到输出的目录当中
>
> <img src="./assets/image-20240521154623134.png" alt="image-20240521154623134" style="zoom:67%;" />
>
> 导出 URL 地址：我们写的路径中，会被webpack进行翻译处理，然后它会导出一个URL地址，指向你输出的这个图片的路径，包括它给你修改的名字，Webpack都帮我们做完了
>
> ![image-20240127165105134](.\assets\image-20240127165105134.png)

小于 8KB 文件：导出一个 data URI（base64字符串）

> 小于 8KB 就会将图片打包进JS代码中，但是打包进的去的时候它会把图片的数据转成 base64这种格式的字符串，如下图，逗号往后就是转成的base64的字符串，它实际上很长，但是这里显示不下，用桑格点省略了，这个数据还需要配合data:这种格式，然后指定后面内容字符串的内容类型，它是一个base64的图片的数据字符串，而我们这样的一个网址它就叫做data URI（数据统一资源标识符），和URL（统一资源定位符）还是很像的
>
...
那为什么大于8kb的不一起打包进JS中呢？为什么要单独的抽离出来让它发一次请求呢？

其实当我们把资源模块转成base64字符串的时候，可能会让这个文件真实的体积大20%到30%，所以如果这个文件越大，转base64字符串就得不偿失了，因此大文件还是直接复制文件。

---

刚刚我们还查了输出到哪里，就是通过 `generator属性` 中 `filename` 中进行设置，以 `output.path` 为相对路径。

默认输出的位置就是 `out.path` 下。

因此如下图片中的配置就是输出到 `dist/asset` ，`[hash][ext][query]` 是文件的名字。

<img src="./assets/image-20240521155712834.png" alt="image-20240521155712834" style="zoom:67%;" />

`[]` 是Webpack提供的一个固定的占位符。

占位符 `[hash]` 对代码中引入的资源模块的内容做算法计算，这个算法的名字就叫做 hash，得到映射的数字字母组合的字符串
...
> 2. 如果代码中有多个相同的图片，虽然它们的名字不一样，但是当它们在转为hash算法字符串的时候，就会把三张图片变成一张，但是代码中在使用这三种图片的时候，它都会使用同一个URL

占位符 【ext】使用当前模块原本的扩展名，例如：`.png / .jpg` 等字符串

占位符 【query】保留引入文件时代码中查询参数（只有 URL 下生效），保留的就是 `?` 后面的查询参数

> 这个用的特别少，了解即可
>
> 这种模式只有在输出单独的文件，引网址才有效，如果转成了js代码当中，这种格式是不生效的

介绍了这么多，其实最关键的就是将配置加入到我们自己的项目中。
...
1. 在less中使用了一张大于8kb的图片

   ![image-20240521161510132](./assets/image-20240521161510132.png)

   ![image-20240521161349154](./assets/image-20240521161349154.png)

2. 在 src/login/index.js 中给 img 标签添加 logo 图片（小于8kb）

   ```js
   // PS：这里面我们不能直接将图片路径给img标签的src，因为Webpack会把src的路径字符串当成普通的字符串原封不动的给你打包，它并不会把图片文件引过来
   // 所以要注意：js 中引入本地图片资源要用 import 方式（如果是网络图片http地址，字符串可以直接写在src即可，运行在浏览器上时，浏览器发现运行的是外网的地址，就能正常请求）
   // Webpack在解析这句话时，它发现图片如果小于8kb，那么打印这个变量的值就是转化后的base64的字符串
...
4. 打包后运行观察效果
   ...
   可以发现大于8kb的发送了一个单独的文件到我们指定的输出目录

   <img src="./assets/image-20240521162952618.png" alt="image-20240521162952618" style="zoom:67%;" />

   小图就被打包进了JS中

   <img src="./assets/image-20240521163107426.png" alt="image-20240521163107426" style="zoom:67%;" />

   然后在浏览器中打开html文件检查确认一下：小图是base64格式的

   ![image-20240521163206506](./assets/image-20240521163206506.png)

   大图就是这种网址，由于这里是本地打开的html网页，因此网址是这样的

   ![image-20240521163417588](./assets/image-20240521163417588.png)

   但是如果是运行了一个服务器，那么网址就会如下显示
...
之前虽然讲过了Webpack打包，但是每次改动代码，都要重新打包，很麻烦，所以一般在开发项目的时候，都会采用开发环境来进行项目的编写。在Webpack中搭建开发环境就需要借助`webpack-dev-server` 包。

这个包的作用作用：会在自己的计算机本地启动一个 Web 服务，它会自动检测代码变化，如果有变化它就会自动重新打包，并且以非常快的速度热更新到我们浏览的网页上。

热更新：代码变化后，它会直接替换变化的代码，自动更新网页。（而没有变化的地方就不会替换）

**PS：dist目录和打包内容是在内存里（更新快）**
...
- 开发环境：即我们这个项目还处于自己电脑中开发代码的环境。

- 生产环境：代码部署到线上，给我们网络用户访问的那个电脑中的环境

首先需要配置之前在终端中看见的 `mode` 警告

![image-20240127181850464](.\assets\image-20240127181850464.png)

这次的值要用 `development`，生产环境就是 `production`，这两个英语单词是固定的。

<img src="./assets/image-20240521171128252.png" alt="image-20240521171128252" style="zoom:67%;" />
...
module.exports = {
    // 首先配置之前在终端中看见的mode的警告，这一次它的值需要使用，
    mode: 'development',
    entry: {
        index: './src/index.js',
        print: './src/print.js',
    ...
    plugins: [
        new HtmlWebpackPlugin({
            // 这个是HtmlWebpackPlugin插件产生的网页上的标题，可以设置也可以不设置
...
接着往下看，选择一个开发工具：第一个可以单独使用和尝试一下，第三个其实被包含在第二个中

<img src=".\assets\image-20240127182056787.png" alt="image-20240127182056787" style="zoom:50%;" />

这里就采用 `webpack-dev-server`，它会为我们提供一个web服务，具有实时的重新加载的功能。

它也是一个软件包，因此也需要先下载。

![image-20240521171344798](./assets/image-20240521171344798.png)

然后就是修改配置文件，`devServer` 这个配置文档中有介绍：告诉我们这个软件包启动的服务，将我们dist目录下的内容部署到 `8080服务`下，也就是说dist目录下的内容会作为我们启动的web服务的静态的资源模块，也就是说我们通过 `localhost:8080然后/` 就可以访问这个dist目录下的任意的静态资源的内容。

![image-20240127182322074](.\assets\image-20240127182322074.png)

但是如果阅读官方文档依旧不懂devServer是什么意思，可以直接进行搜索，此时就会来到devServer相关的设置当中。

![image-20240127182645366](.\assets\image-20240127182645366.png)

在这里面我们就可以找到static，如果我们不设置static，它的默认值就是 `public文件夹`

![image-20240127182823119](.\assets\image-20240127182823119.png)

如果看到这还是不太明白，此时就需要结合百度文章，然后再回到自己的代码中不断地尝试不断地调整，进行一个分析。

static我们现在设置和不设置都可以，所以我们这里暂时不用，因为这个东西webpack-dev-server它还是要借助Webpack打包我们编写的代码，但是这一次它把代码打包在了内存当中，所以磁盘中有没有dist目录其实都不是必须的。
...
2. 配置打包的模式为开发模式，并设置自定义命令
   ...
   因为这个包要求我们当前的Webpack项目必须在 `mode模式` 为 `development` 下才能运行。
...
>
> 当它发现public文件夹下面只有一个login.html，其实访问文件夹它会默认找index.html固定名字的网页，但是没有，所以它就显示了这样的一个网页
...
但是当你访问 `localhost:8080/login.html` 的时候，会发现图片、背景、样式等都没加载出来

<img src="./assets/image-20240521174040906.png" alt="image-20240521174040906" style="zoom:67%;" />

此时就需要说到另外几个注意事项了。

~~~js
// 注意3：webpack-dev-server 根据配置，打包相关代码在内存当中，以 output.path 的值作为服务器根目录，所以它的根目录有两个，一个是默认的静态资源public，另一个是output.path
// 因此实际上dist打包的资源其实也在服务器的根目录中，所以可以直接自己拼接访问 dist 目录下内容
// 由于我们是打包在内存中，所以打包好后保存在内存中的dist和VSCode中的dist一毛钱关系也没有
console.log('观察页面是否有自动打包更新')
~~~

直接拼接之前设置的HTML页面输出路径，即：`locaohost:8080/login/index.html`

![image-20240127184614932](.\assets\image-20240127184614932.png)

如果两个根目录里的内容有冲突，则首先访问public下的，如果想让打包之后它能够自动命中我们输出的登录页面

在public下创建一个 `index.html`（因为webpack默认以public文件夹作为服务器根目录），来所以打开之后，它就会直接访问index.html
...
<body>
  <script>  
    // 作用：让 webpack-dev-server 的 Web 服务，自动跳转到我们规定的首页
    location.href = '/login/index.html'
  </script>
</body>
</html>
...
[打包模式](https://webpack.docschina.org/configuration/mode/) 作用：告知 Webpack 使用相应模式的内置优化方案，进行打包后产物的优化
...
| **模式名称** | **模式名字** | **特点**                                                     | 场景     |
| ------------ | ------------ | ------------------------------------------------------------ | -------- |
| 开发模式     | development  | 调试代码，实时加载，模块热替换（即只替换变化的模块到浏览器上）等（快） | 本地开发 |
| 生产模式     | production   | 压缩代码，资源优化，更轻量，适配不同的浏览器环境等（体积更小） | 打包上线 |
...
注意：命令行设置（即方式2）的优先级高于配置文件（方式1）中的，推荐用命令行设置，不然每次想要修改，还需要到配置文件中修改。
...
> PS：由于dev打包好的都放在内存中，我们也看不见打包后的代码，所以体验的时候就使用build来体验
>
> <img src=".\assets\image-20240128114107268.png" alt="image-20240128114107268" style="zoom:67%;" />

production会极致压缩，将 `build` 修改mode值为 `production`，生成的代码如下

![image-20240521175932413](./assets/image-20240521175932413.png)

development会把代码分成一个个的模块，在同一个文件里会分成一段一段的，好处是某一个文件的代码发生变化，直接替换对应名字的这段代码就行了。将 `build` 修改mode值为 `development`，生成的代码如下

![image-20240521180110865](./assets/image-20240521180110865.png)

`build` 的 `mode值` 还是改为 `production`，让它体积更小一点，`dev` 是开发环境，因此设置为 `development`，让它模块热替换更快一点，不要求极致的体积压缩。

<img src="./assets/image-20240521180334958.png" alt="image-20240521180334958" style="zoom:67%;" />
...
来到配置文件中，找到插件生成CSS文件的地方，传入filname。

PS：这个插件要求你只能传入相对路径，所以我们不能像HTML这个插件这样写。

它会以出口目录 `dist目录`，然后接着拼接即可

![image-20240521180522340](./assets/image-20240521180522340.png)

重新执行 `npm run build` 进行打包，可以发现原来在 dist目录下的main.css进去了

<img src="./assets/image-20240521180710807.png" alt="image-20240521180710807" style="zoom:80%;" />
...
- 在开发模式下用 `style-loader` 来进行CSS样式的处理，让CSS代码内嵌在JS文件中，可以让我们在开发时，如果样式发生了修改，它可以更快的替换到网页上。

- 在生产环境下想要将css代码单独提取成一个CSS文件，为了让我们打包输出后的代码在用户的浏览器上可以并行下载CSS和JS代码，所以这样的需求我们应该怎么办呢？

想要做这样的区分，就需要判断一下当前当前运行命令时所在的环境，想做这样的判断区分，我们有三种方案。

---

## 二、方案一

方案一：让 `webpack.config.js` 不再导出一个对象，而是导出一个函数，在函数的形参上我们就能访问之前设置的 `mode属性` 的值了。

查看官方文档：https://webpack.docschina.org/configuration/mode/

点进文档后，来到文档的配置模式里面，可以快捷模式一共支持一下几种值

<img src="./assets/image-20240521182023370.png" alt="image-20240521182023370" style="zoom:67%;" />

然后再往下翻，就可以找到：如果要根据 *webpack.config.js* 中的 **mode** 变量更改打包行为（即需求中选择不同的loader加载器），则必须将配置导出为函数，而不是导出对象。
...
~~~js 
var config = {module.exports = (env, argv) => {
